...
;(function(root, factory) {
	...
	var $ = window.jQuery,	// change @tag
			jet = {}, doc = document, ymdMacth = /\w+|d+/g, parseInt = function (n) { return window.parseInt(n, 10);},
	config = {
		skinCell:"jedateblue",
	...
	// create @tag
	jet.setValOrTxt = function( that, val ) {
		var _field = jet.isValHtml( that ) ? 'val' : 'text',
				_val = that[_field]()

		that[_field](val)
		// 如果值不同就触发change事件, 兼容支持ng双向绑定
		if( _val !== val ) {
			that.trigger('change')

			// 兼容jq.validate
			if( typeof that.rules == 'function' )
	...
	jedfn.init = function(){
		...
		if (jet.isBool(opts.insTrigger)) {
			that.valCell.off('click.jedate').on("click.jedate", function (ev) {
				ev.stopPropagation();
...
//			leris = rect.right + obj.outerWidth() / 1.5 >= jet.winarea(1) ? rect.right - obj.outerWidth() : rect.left + (pos ? 0 : jet.docScroll(1));
		  leris = rect.left + obj.outerWidth() >= jet.winarea(1) ? rect.right - obj.outerWidth() : rect.left + (pos ? 0 : jet.docScroll(1)); // change @tag
	...
	jedfn.setHtml = function(opts){
		...
		var bothmsStr = !isYYMM ? '<div class="botflex jedatehmsshde"><ul class="jedatehms"><li><input type="text" /></li><i>:</i><li><input type="text" /></li><i>:</i><li><input type="text" /></li></ul></div>' + '<div class="botflex jedatebtn"><span class="jedateok">确认</span>'+ ((jet.minDate || jet.maxDate) && (Date.parse(jet.minDate) > date.getTime() || Date.parse(jet.maxDate) < date.getTime()) ? "" : '<span class="jedatetodaymonth">今天</span>') +'<span class="jedateclear">清空</span></div>' :(dateFormat == "YYYY" ? '<div class="botflex jedatebtn"><span class="jedateok" style="width:47.8%">确认</span><span class="jedateclear" style="width:47.8%">清空</span></div>' : '<div class="botflex jedatebtn"><span class="jedateok">确认</span><span class="jedatetodaymonth">本月</span><span class="jedateclear">清空</span></div>');
	...
	jedfn.setStrhms = function(opts) {
        ...
        var minhms = (jet.minDate.split(" ")[1] && jet.minDate.split(" ")[1].match(ymdMacth) )|| jet.minDate.match(ymdMacth),
            maxhms = (jet.maxDate.split(" ")[1] && jet.maxDate.split(" ")[1].match(ymdMacth) )|| jet.maxDate.match(ymdMacth),
		...
		$.each([ 24, 60, 60 ], function(i, len) {
			var hmsStr = "", hmsCls = "", inputCls = boxCell.find(".jedatehms input").prop("readonly",true), textem = inputCls.eq(i).val();
			inputCls.eq(i).attr("maxlength",2).attr("numval",len-1).attr("item",i);
			for (var h = 0; h < len; h++) {
				h = jet.digit(h);
				if (jet.isBool(opts.hmsLimit)) {
                    hmsCls = parmathm && i == 2 ? (minhms[i] || "00") == h ? "disabled action" :"disabled" :textem == h ? "action" : "" ;
                    if(h < minhms[i] ){
                    	hmsCls +=" disabled";
                    }
                    if(h > maxhms[i]){
                    ...
                    if(h == maxhms[i] && maxhms.length == 2 && ( maxhms[1] == "00" || i == 1) ){
                    	hmsCls +=" disabled";
                    }
                    if(h == minhms[i] && minhms.length == 2 && (minhms[1] == "00" || i == 1) ){
                    	hmsCls +=" disabled";
                    }
                    if( (inputCls.eq(0).val() < maxhms[0] ) && maxhms.length == 2 && i == 1){
                    	hmsCls = hmsCls.replace("disabled","");
                    }

                     if( (inputCls.eq(0).val() == maxhms[0] ) && i == 1){
                     	if(textem > maxhms[1]){
                     		if(h + 1 == maxhms[1] ){
	                     	 	hmsCls += " action"
	                     	 	inputCls.eq(1).val(h)
	                     	 }else{
	                     	 	hmsCls = hmsCls.replace("action","");
	                     	 }
                     	}else{
                     		hmsCls = hmsCls.replace("action","");
                     		if( h + 1 == maxhms[1]){
                      ...
                      if( (inputCls.eq(0).val() == minhms[0] ) && i == 1){
                      	if(textem < minhms[1]){
                     		if(h - 1 == minhms[1] ){
	                     	 	hmsCls += " action"
	                     	 	inputCls.eq(1).val(h)
	...
	jedfn.chooseDays = function(opts) {
		...
		boxCell.find(".jedaul li").on("click", function(ev) {
			...
			jet.setValOrTxt(elemCell, getDateVal)
			that.dateClose();
	...
	jedfn.ymPremNextEvents = function(opts){
		...
		ymCls.on("click", function (ev) {
			...
			jet.setValOrTxt(elemCell, getYMDate)
			that.dateClose();
	...
	jedfn.events = function(tmsArr,opts){
		...
		if (!isYYMM) {
			...
			var inputCls = boxCell.find(".jedatehms input")
				if (ishhmmss) {
					jet.isShow(boxCell.find(".jedatehmsclose"), false);
					jet.isShow(boxCell.find(".jedatetodaymonth"), false);
					var minhms = jet.minDate.match(ymdMacth),
                        maxhms = jet.maxDate.match(ymdMacth);
				} else {
				...
				$.each([ "hours", "minutes", "seconds" ], function(i, hms) {
					...
					var onhmsPCls = boxCell.find(".jedateprop" + hms);

					onhmsPCls.on("click","p",function() {
						var _this = $(this);
						...
						if(i == 0){
					        ...
					        pies = boxCell.find(".jedatepropminutes p");
							if(_this.text() < maxhms[0]){
								pies.removeClass("disabled");
							}else if(_this.text() == maxhms[0]) {
								pies.each(function(){
									var _this = $(this);
									if(_this.text() >= maxhms[1]){
										_this.addClass("disabled");
										if( _this.text() == maxhms[1] ) {
											_this.prev().addClass("action").siblings().removeClass("action");
											inputCls.eq(1).val( jet.digit(_this.text() - 1 ) );
										}
									}

								})
							}
							if(_this.text() > minhms[0]){
								pies.removeClass("disabled");
							}else if(_this.text() == minhms[0]) {
								pies.each(function(){
									var _this = $(this);
									if(_this.text() <= minhms[1]){
										_this.addClass("disabled");
										if( _this.text() == minhms[1] ) {
											_this.next().addClass("action").siblings().removeClass("action");
											inputCls.eq(1).val( jet.digit(_this.text() - 0 + 1 ) )
										}
									}
								})
							}
			...
			boxCell.find(".jedatebot .jedatetodaymonth").on("click", function() {
				...
				jet.setValOrTxt( elemCell, gettoDate )
				that.dateClose();
			...
			boxCell.find(".jedatebot .jedatetodaymonth").on("click", function(ev) {
				...
				jet.setValOrTxt( elemCell, YMDate )
				that.dateClose();
		...
		boxCell.find(".jedatebot .jedateok").on("click", function(ev) {
			...
			jet.setValOrTxt(elemCell, okVal)
			that.dateClose();
...
function RangeDate( $ele1, $ele2, opt ) {
	var opt1, opt2

	opt1 = $.extend( {}, RangeDate.defaulOpt, {
		choosefun: function(ele, dates) {
			opt2.minDate = dates
			console.log(dates)
		},
		okfun: function(ele, dates) {
	    	opt2.minDate = dates
	    },
	    clearfun:function(ele, dates) {
	        opt2.minDate = ""
	    }
	}, opt )

	opt2 = $.extend( {}, RangeDate.defaulOpt, {
	...
	if( '' != $ele1.val() && $.jeDateMethod.checkDate($ele1.val()) ) {
		opt2.minDate = $ele1.val() + ' '
	}
	if( '' != $ele2.val() && $.jeDateMethod.checkDate($ele2.val()) ) {
		opt1.maxDate = $ele2.val() + ' '
	}
...
$.fn.rangeJeDate = function( $elem, opt ) {
	var instance = $elem.data('rangeJeDate')
	if( instance == null ) {
		instance = new RangeDate( this, $elem, opt)
		this.data( 'rangeJeDate', instance )
		$elem.data( 'rangeJeDate', instance )
	}
}
